...
$.fn.themeswitcher = function (settings) {
	var options = jQuery.extend({
			...
			useStandard : true, 
			cssPrefix : "~/res/BaseClasses/Scripts.aspx?f=jQueryLibrary/", 
			cssSuffix : ".css", 
			imgPrefix : "~/res/BaseClasses/Scripts.aspx?f=JqueryUIControls/", 
			imgSuffix : ".png", 
			imageLocation : "" , 
			themes : {
			}, 
...
if (options.useStandard)
	...
	for (var i in options.themes) {
		css = options.themes[i].css || options.cssPrefix + i + "." + i + options.cssSuffix;
	...
	theme_ul = "<ul>" + theme_ul + "</ul>"
		...
		//markup 
		var button = $('<a href="#" class="jquery-ui-themeswitcher-trigger"><span class="jquery-ui-themeswitcher-icon"></span><span class="jquery-ui-themeswitcher-title">' + 
	...
	var button_default = {
		...
		fontSize : '11px', 
		color : '#666', 
		background : '#eee url(' + options.imgPrefix + 'buttonbg.png) 50% 50% repeat-x', 
	...
	.find('.jquery-ui-themeswitcher-icon').css({
			...
			width : '16px', 
			height : '16px', 
			background : 'url(' + options.imgPrefix + 'icon_color_arrow.gif) 50% 50% no-repeat'
		});
	//pane css
	switcherpane.css({
			position : 'absolute', 
	...
	.find('li').hover(
		function () {
			$(this).css({
					'borderColor' : '#555', 
					'background' : 'url(' + options.imgPrefix + 'menuhoverbg.png) 50% 50% repeat-x', 
					cursor : 'pointer'
				});
		}, 
		function () {
