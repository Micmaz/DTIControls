	...
	$.extend($.summernote.plugins, {
		...
		'dialogHelper': function (context) {
			...
			var createCustDialog = function (title, body, callbackFunction, dialogButtonText) {
				...
				var $container = options.dialogsInBody ? $(document.body) : $('.panel-default').first();
			...
			var createIframeDialog = function (title, iframeLocation, dialogButtonText) {
				...
				$('#' + diagID).load(function () {
					// Safari and Opera need a kick-start.
					//var iSource = document.getElementById(diagID).src;
					//document.getElementById(diagID).src = '';
					//document.getElementById(diagID).src = iSource;
					setTimeout(function () {
						document.getElementById(diagID).style.height =
							document.getElementById(diagID).contentWindow.document.body.offsetHeight + 10 + 'px';
					}, 50);
				});
			...
			return {
				'createIframeDialog': createIframeDialog,
