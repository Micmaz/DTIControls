...
#Region "Shared Attributes"
	...
	''' <summary>
	''' Returns True when the application was deployed as a precompiled site (aspnet_compiler). Precompiled
	''' sites ignore RegisterVirtualPathProvider and cannot compile virtual .aspx/.ascx at runtime; the
	''' embedded resources are served instead from the precompiled App_Web assemblies and .compiled manifests
	''' generated by _DTIControls\PrecompileEmbeddedPages.ps1 and deployed to the site's bin folder.
	''' </summary>
	''' <remarks></remarks>
	<System.ComponentModel.Description("Returns True when the application was deployed as a precompiled site (aspnet_compiler).")>
	Public Shared ReadOnly Property isPrecompiledApp() As Boolean
		Get
			If Not _isPrecompiledApp.HasValue Then
				Try
					Dim configPath As String = System.Web.Hosting.HostingEnvironment.MapPath("~/PrecompiledApp.config")
					_isPrecompiledApp = configPath IsNot Nothing AndAlso System.IO.File.Exists(configPath)
				Catch ex As Exception
					_isPrecompiledApp = False
				End Try
			End If
			Return _isPrecompiledApp.Value
		End Get
	End Property
	Private Shared _isPrecompiledApp As Nullable(Of Boolean)
	...
	Public Shared Sub registerVirtualPathProvider()
		If isPrecompiledApp Then
			'The runtime ignores RegisterVirtualPathProvider in precompiled sites, so don't register (or
			'retest) anything: the precompiled /res/... manifests in bin serve the embedded pages instead.
			'The resource cache is still warmed once so Scripts.ashx and ListResources keep serving static
			'embedded content (js/css/images), which never needed page compilation.
