...
Public Class AssemblyLoader
    ...
    Public Shared Function CreateInstance(ByVal classname As String, Optional ByVal isretry As Boolean = False, Optional ByVal baseType As Type = Nothing) As Object
        ...
        For i As Integer = 0 To assemblies.Count - 1
            Dim asm As Assembly = assemblies(i)
				'Try
				''Dim myType As Type = asm.GetType(classname)
				obj = asm.CreateInstance(classname, True)
				'If baseType IsNot Nothing AndAlso obj Is Nothing Then
				'	Dim constInfo As ConstructorInfo = baseType.GetConstructor(Type.EmptyTypes)
				'	obj = constInfo.Invoke(New Object() {})
				'End If
    ...
    Public Shared Function runM(ByVal o As Object, ByVal methodname As String, ByVal ParamArray args() As Object) As Object
        ...
        Else
			...
			Try
				prop = o.GetType().GetProperty(methodname, BindingFlags.Instance Or BindingFlags.NonPublic Or BindingFlags.Public Or BindingFlags.IgnoreCase)
			Catch ex As System.Reflection.AmbiguousMatchException
				For Each p As PropertyInfo In o.GetType().GetProperties(BindingFlags.Instance Or BindingFlags.NonPublic Or BindingFlags.Public Or BindingFlags.IgnoreCase)
					If p.Name.ToLower() = methodname.ToLower() Then prop = p
				Next
			End Try
