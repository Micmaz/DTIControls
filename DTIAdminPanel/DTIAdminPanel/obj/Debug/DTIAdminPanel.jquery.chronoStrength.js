...
**
*********************************************************/
function ChronoStrengthGo(txtid,cssClass)
{		
	...
	$("#" + txtid).keyup(function(e){passwordCheck(this, e, cssClass);});
}

function passwordCheck(that, e, cssClass)
{
	...
	if (password != "")
	{		
		var strength = $.chronoStrengthWithCustomSpeed(password, 250000000);
		
		if (strength == 'One of the 500 most common passwords')
		{
			$("div." + cssClass).html('<span class="top-message">Your password is</span> <span class="password-strength">' + strength + '.</span> <span class="bottom-message">You should probably change it.</span>').fadeIn(500);
		}
		else if (strength == 'Instantly')
		{
			$("div." + cssClass).html('<span class="top-message">Your password would be cracked</span> <span class="password-strength">Instantly.</span> <span class="bottom-message">You should probably change it.</span>').fadeIn(500);
		}
		else
		{
			$("div." + cssClass).html('<span class="top-message">It would take</span> <span class="password-strength">' + strength + '</span> <span class="bottom-message">for a desktop PC to crack your password by brute force.</span>').fadeIn(500);
		}
	}
	else
	{
...
function chronoStrengthPasswordStrength(password, calculationsPerSecond)
{
	...
	// Unicode Cyrillic
	if (password.match(/[\u0410-\u042F]+/)) {possibleCharacters += 32};
	if (password.match(/[\u0430-\u044F]+/)) {possibleCharacters += 32};
	
		
	// Dictionary test
	var word = false;
	if (password.match(/^[a-z]+$|^[A-Z]+$/)) { word = true; }
	...
	if (computerTimeInSecs < 1)
	{
		if(computerTimeInSecs > 0.000001)
		{
			strength = computerTimeInSecs+" seconds";
		}
		else
		{
			strength = 'Instantly';
