...
(function($) {
	...
	$.tree = {
		...
		defaults	: {
			...
			rules	: {
				...
				no_int_copy : true,     // true | false - disables copy on the same tree
				auto_insert_on_drop: true,
				use_max_children	: true,
				use_max_depth		: true,
				fill_hidden_for_postback: true,
				new_node_name       : "New_Node",
				new_node_count      : 0,  //for use internally
	...
	function tree_component () {
		return {
			...
			init : function(elem, conf) {
				...
				$.tree.tree_ids.push(this.container.attr("id"));
				...
				if(this.settings.ui.theme_name !== false) {
					...
					//do the obvious of add the the convention used above to a different theme path
					else if(this.settings.ui.theme_path.indexOf(this.settings.ui.theme_name) < 0){ this.settings.ui.theme_path += "/" + this.settings.ui.theme_name + "/style.css";}
			...
			get_text : function (obj, lang) {
				...
				obj.contents().each(function () {
					if(this.nodeType == 3) { val = this.data;}
				});
				return val;
			},
			get_nodes : function (){
				if(this.locked) return false;
				var len = $("#" + this.container.context.id + " > ul").children().length;
				var i = 0;
				var elements = new Array();
				var ele;
				for (i=0; i<len; i++){
					var node = $("#" + this.container.context.id + " > ul").children()[i];
					this.add_nodes_to_array(node, elements, null);
				}
				return elements;
			},
			add_nodes_to_array: function (NODE, ELEMENTS, ParentId){
				if(this.locked) return false;
				ELEMENTS.push(this.get_node_data(NODE, ParentId));
				var nodes = this.children(NODE);
				var len = nodes.length;
				var i=0;
				for (i=0; i<len; i++){
					this.add_nodes_to_array(nodes[i], ELEMENTS, NODE.id);
				}
			},
			get_node_data: function (NODE, ParentID){
				if(this.locked) return false;
				ele = new Array();
				//a.classname is checked for checked
				var checked = "None";
				checked = ($(NODE).children().filter("a:eq(0)").attr("className").indexOf("checked") > -1)? "checked" : checked;
				//a.classname is undetermined for undetermined
				checked = ($(NODE).children().filter("a:eq(0)").attr("className").indexOf("undetermined") > -1)? "undetermined" : checked;
				//a.classname is clicked for selected
				var selected = ($(NODE).children().filter("a:eq(0)").attr("className").indexOf("clicked") > -1);
				//NODE.classname is open or last open if opened
				var open =  (NODE.className == "open" || NODE.className == "last open");
				ele.push(NODE.id, ParentID, $(NODE).attr("key"), this.get_text(NODE), checked, selected, open, $(NODE).attr("rel"));
				return ele;
			},
			...
			create : function (obj, ref_node, position, rootType, forceCreate) { 
				if(this.locked) return this.error("LOCKED");
				
				var root = false;
				var replaceRoot = false;
				if(ref_node == -1) { root = true; ref_node = this.container; }
                //check for empty tree holder
				if ($(ref_node).attr("rel") == "EmptyTreeHolder"){
				    root = true; ref_node = this.container; replaceRoot = true; this.remove(ref_node, true);
				}
				...
				if(!root) {
					if(!this.check("creatable", ref_node) && !forceCreate) return this.error("CREATE: CANNOT CREATE IN NODE");
				...
				if(this.settings.languages.length) {
					...
					if(!obj.data) { 
					    if(root && !replaceRoot){
					        obj.data = this.settings.lang.tree_empty; 
					    }
					    else{
				...
				var $li = $(obj_s);
				if(root && !replaceRoot && !rootType){
					$li.attr("rel", "EmptyTreeHolder");
				    $li.attr("key", "-1");
				}else if ($li.attr("key") == null){
				    $li.attr("key", "-2");
				}
				$li.attr("id", this.settings.rules.new_node_name + "_" + this.settings.rules.new_node_count++);
			...
			remove : function(obj, force) {
				...
				if(obj && (!this.selected || this.get_node(obj).get(0) != this.selected.get(0) )) {
					obj = this.get_node(obj);
					if(obj.size()) {
						if(!this.check("deletable", obj) && !force) return this.error("DELETE: NODE NOT DELETABLE");
						...
						//Add to delete array
						try{
						    this.settings.data.deleted.push(obj.attr("id"));}
						catch(err){;}
						...
						//if all nodes are gone add empty tree node
						if(!$(this.container[0]).find("li").length){
						    this.create(null, -1, null);
						}
			...
			moved : function (what, where, how, is_new, is_copy, rb) {
				...
				if($where.parent().attr("rel") == "EmptyTreeHolder"){how="before";}
				...
				// Check old parent for 0 nodes and add place holder
				if(!$($($parent.context)).find("li").size()) {
			        $.tree.reference($parent).create(null, -1, null);
				}
				// CLEANUP OLD PARENT
                ...
                if($where.parents("li:eq(0)").attr("rel") == "EmptyTreeHolder"){
                    this.remove($where.parents("li:eq(0)"), true);
                }
				...
				else if(is_new)	this.callback("oncreate", [this.get_node(what).get(0), ($where.is("ul") ? this.get_node(where).get(0) :  -1), how, this, rb]);
	...
	tree_component.mouseup = function(event) {
		...
		if(tmp.ref_node){
            ...
            if (tree1) tmp.foreign = tree1 !== tmp.origin_tree;
        }
		if(tmp.foreign === false && tmp.drag_node && tmp.drag_node.size()) {
			tmp.drag_help.remove();
			if(tmp.move_type) {
				if(tree1) tree1.moved(tmp.dragged, tmp.ref_node, tmp.move_type, false, ((tmp.origin_tree.settings.rules.drag_copy == "on" && (tmp.origin_tree.settings.rules.no_int_copy?(tree1 !== tmp.origin_tree):true)) || (tmp.origin_tree.settings.rules.drag_copy == "ctrl" && event.ctrlKey) ) );
			}
			tmp.move_type	= false;
			tmp.ref_node	= false;
		}
		if(tmp.foreign !== false) {
			...
			if(tmp.move_type) {
				if(tree1) {
				    if(tree1.settings.rules.auto_insert_on_drop){
				        tree1.moved(tmp.dragged, tmp.ref_node, tmp.move_type, false, ((tmp.origin_tree.settings.rules.drag_copy == "on" && (tmp.origin_tree.settings.rules.no_int_copy?(tree1 !== tmp.origin_tree):true)) || (tmp.origin_tree.settings.rules.drag_copy == "ctrl" && event.ctrlKey) ) );
				    }
				    tree1.callback("ondrop",[tmp.dragged, tree1.get_node(tmp.ref_node).get(0), tmp.move_type, tree1]);
				}
			}
			tmp.foreign		= false;
			tmp.move_type	= false;
...
$(document).ready(function(){
    $("form").submit(function() {
        var i;
        for (i=0; i < $.tree.tree_ids.length; i++){
            elId = $.tree.tree_ids[i];
            try{
